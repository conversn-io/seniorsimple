# 🕷️ SeniorSimple Site Crawler Agent

Run the comprehensive site crawler specifically for SeniorSimple that discovers broken links and UX issues.

## 🚀 Usage Instructions:

### **Option 1: CrewAI Agent (Recommended)**
```bash
# Run the CrewAI agent that automatically crawls SeniorSimple
...
2. **Navigate to SeniorSimple**: `http://localhost:3010`
...
## 🎯 SeniorSimple Specific Tests:

- ✅ **Category Pages**: `/retirement`, `/housing`, `/health`, `/estate`, `/tax`, `/insurance`
- ✅ **Quiz Flow**: Home Equity Quiz functionality
- ✅ **Mobile Menu**: Navigation accessibility
- ✅ **Touch Targets**: Mobile-friendly button sizes
- ✅ **CallReady Tracking**: Lead capture integration
 ...
 * 🕷️ SENIORSIMPLE SITE CRAWLER AGENT
 * 
 * This agent crawls the SeniorSimple site to discover:
 ...
 * 
 * Usage: Navigate to http://localhost:3010 and run this script
 * Works on both localhost and production automatically!
 */

console.log('🕷️ Starting SeniorSimple Site Crawler Agent...');
...
const CRAWLER_CONFIG = {
  baseUrl: baseUrl,
  testSessionId: `seniorsimple_crawl_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
  delayBetweenActions: 1500,
  maxRetries: 3,
  timeout: 10000,
  testUser: {
    firstName: 'SeniorSimple',
    lastName: 'Test',
...
// Test SeniorSimple specific pages
async function testSeniorSimplePages() {
  logCrawlStep('SeniorSimple Pages', 'Testing SeniorSimple specific pages...');
  ...
  try {
    ...
    logCrawlSuccess('SeniorSimple Pages', `Tested ${CRAWLER_CONFIG.targetPages.length} pages`);
    return true;
    
  } catch (error) {
    logCrawlError('SeniorSimple Pages', 'SeniorSimple page testing failed', error);
    return false;
  }
}
...
function generateComprehensiveCrawlReport() {
  ...
  console.log('\n' + '='.repeat(80));
  console.log('🕷️ SENIORSIMPLE SITE CRAWLER REPORT');
...
async function runComprehensiveSiteCrawler() {
  logCrawlStep('Site Crawler', 'Starting SeniorSimple Site Crawler...');
  ...
  try {
    ...
    logCrawlStep('Site Crawler', '✅ SeniorSimple site crawling completed!');
    
    return report;
    
  } catch (error) {
    logCrawlError('Site Crawler', 'SeniorSimple site crawling failed', error);
    return CRAWLER_RESULTS;
  }
}

// Export for manual testing
window.seniorsimpleSiteCrawler = {
  runTest: runComprehensiveSiteCrawler,
  testURLAccessibility: testURLAccessibility,
  testSeniorSimplePages: testSeniorSimplePages,
...
console.log('🕷️ SeniorSimple Site Crawler functions available:');
console.log('   - window.seniorsimpleSiteCrawler.runTest()');
console.log('   - window.seniorsimpleSiteCrawler.testURLAccessibility(url)');
console.log('   - window.seniorsimpleSiteCrawler.testSeniorSimplePages()');
console.log('   - window.seniorsimpleSiteCrawler.testNavigationElements()');
console.log('   - window.seniorsimpleSiteCrawler.testFormFunctionality()');
console.log('   - window.seniorsimpleSiteCrawler.testPerformanceMetrics()');
console.log('   - window.seniorsimpleSiteCrawler.testMobileResponsiveness()');
console.log('   - window.seniorsimpleSiteCrawler.testSEOElements()');
console.log('   - window.seniorsimpleSiteCrawler.testCallReadyIntegrationPoints()');
...
- ✅ **SeniorSimple specific issues** and recommendations

The agent will automatically crawl your SeniorSimple site and identify all issues that need to be fixed!

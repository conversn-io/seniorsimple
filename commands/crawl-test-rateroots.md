# 🕷️ RateRoots Site Crawler Agent

Run the comprehensive site crawler specifically for RateRoots that discovers broken links and UX issues.

## 🚀 Usage Instructions:

### **Option 1: CrewAI Agent (Recommended)**
```bash
# Run the CrewAI agent that automatically crawls RateRoots
cd /Users/funkyfortress/Documents/01-ALL\ DOCUMENTS/05\ -\ Projects/CallReady
./scripts/crawler-crewai-agent.py --target rateroots
```

### **Option 2: Manual Browser Method**
1. **Copy the script below**
2. **Navigate to RateRoots**: `http://localhost:3000`
...
## 🎯 RateRoots Specific Tests:

- ✅ **Quiz Flow**: Home Equity Quiz functionality
- ✅ **Funnel Pages**: Lead capture and conversion flows
- ✅ **OTP Verification**: Phone number validation
- ✅ **API Endpoints**: Backend connectivity
- ✅ **Form Submissions**: Lead data capture
- ✅ **CallReady Integration**: Lead processing pipeline
- ✅ **Supabase Integration**: Database operations
- ✅ **GHL Webhooks**: CRM integration
 ...
 * 🕷️ RATEROOTS SITE CRAWLER AGENT
 * 
 * This agent crawls the RateRoots site to discover:
 ...
 * 
 * Usage: Navigate to http://localhost:3000 and run this script
 * Works on both localhost and production automatically!
 */

console.log('🕷️ Starting RateRoots Site Crawler Agent...');

// Auto-detect environment (localhost vs production)
const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
const port = window.location.port || '3000';
const baseUrl = isLocalhost ? `http://localhost:${port}` : 'https://rateroots.com';
...
const CRAWLER_CONFIG = {
  baseUrl: baseUrl,
  testSessionId: `rateroots_crawl_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
...
// Test RateRoots specific pages
async function testRateRootsPages() {
  logCrawlStep('RateRoots Pages', 'Testing RateRoots specific pages...');
  
  try {
    for (const page of CRAWLER_CONFIG.targetPages) {
      const fullUrl = `${CRAWLER_CONFIG.baseUrl}${page}`;
      await testURLAccessibility(fullUrl);
      await wait(CRAWLER_CONFIG.delayBetweenActions);
    }
    
    logCrawlSuccess('RateRoots Pages', `Tested ${CRAWLER_CONFIG.targetPages.length} pages`);
    return true;
    
  } catch (error) {
    logCrawlError('RateRoots Pages', 'RateRoots page testing failed', error);
    return false;
  }
}
...
function generateComprehensiveCrawlReport() {
  ...
  console.log('\n' + '='.repeat(80));
  console.log('🕷️ RATEROOTS SITE CRAWLER REPORT');
...
async function runComprehensiveSiteCrawler() {
  logCrawlStep('Site Crawler', 'Starting RateRoots Site Crawler...');
  ...
  try {
    ...
    logCrawlStep('Site Crawler', '✅ RateRoots site crawling completed!');
    
    return report;
    
  } catch (error) {
    logCrawlError('Site Crawler', 'RateRoots site crawling failed', error);
    return CRAWLER_RESULTS;
  }
}

// Export for manual testing
window.raterootsSiteCrawler = {
  runTest: runComprehensiveSiteCrawler,
  testURLAccessibility: testURLAccessibility,
  testRateRootsPages: testRateRootsPages,
...
console.log('🕷️ RateRoots Site Crawler functions available:');
console.log('   - window.raterootsSiteCrawler.runTest()');
console.log('   - window.raterootsSiteCrawler.testURLAccessibility(url)');
console.log('   - window.raterootsSiteCrawler.testRateRootsPages()');
console.log('   - window.raterootsSiteCrawler.testNavigationElements()');
console.log('   - window.raterootsSiteCrawler.testFormFunctionality()');
console.log('   - window.raterootsSiteCrawler.testPerformanceMetrics()');
console.log('   - window.raterootsSiteCrawler.testMobileResponsiveness()');
console.log('   - window.raterootsSiteCrawler.testSEOElements()');
console.log('   - window.raterootsSiteCrawler.testCallReadyIntegrationPoints()');
...
- ✅ **RateRoots specific issues** and recommendations

The agent will automatically crawl your RateRoots site and identify all issues that need to be fixed!
